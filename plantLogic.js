...
function snowPeaShooterStartShooting(cell) {
    if (cell.shootInterval) {
        clearInterval(cell.shootInterval);
    }
        ...
        const fly = setInterval(() => {
            ...
            document.querySelectorAll('.enemy').forEach(zombie => {
                ...
                if (hit) {
                    ...
                    if (zombie.slowTimeout) clearTimeout(zombie.slowTimeout);
                    zombie.dataset.hp -= 1;
                    zombie.dataset.slowed = 'true';
                    zombie.slowTimeout = setTimeout(() => {
                        delete zombie.dataset.slowed;
                    }, 3000);
                    if (zombie.dataset.hp <= 0) {
                        zombie.remove();
                    }
            ...
            if (document.body.contains(peaProjectile) && peaProjectile.offsetLeft > window.innerWidth) {
                clearInterval(fly);
                peaProjectile.remove();
            }
        }, 10);
